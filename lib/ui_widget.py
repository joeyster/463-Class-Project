...
class UIWidget(QWidget):
    ...
    def delayed_ui(self):
        ...
        if not self.full_ui_created:
            ...
            pack_warehouse_button.move(142, 510)
            pack_warehouse_button.show()
            self.available_space = QLineEdit('Space Remaining - {}'.format(str(self.size)), self)
            self.available_space.resize(self.available_space.sizeHint())
            self.available_space.move(282, 510)
            self.available_space.setFixedWidth(120)
            self.available_space.setStyleSheet('background-color:rgb(103, 111, 112)' ';color:white')
            self.available_space.setReadOnly(True)
            self.available_space.show()
